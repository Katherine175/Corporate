...
public class DataInitializer {
	...
	static class SeedService {
		...
		private void consumosPrueba() {
			if (consumoRepository.count() > 0) {
				return;
			}
			Usuario admin = usuarioRepository.findByEmail("admin@luxury.com").orElse(null);
			if (admin == null) return;
			
			Sede lima = sedeRepository.findAll().getFirst();
			List<TipoRecurso> recursos = tipoRecursoRepository.findAll();
			String[] periodos = {"2026-03", "2026-04", "2026-05"};
			
			for (String periodo : periodos) {
				for (TipoRecurso recurso : recursos) {
					TarifaRecurso tarifa = tarifaRepository.findAll().stream()
							.filter(t -> t.getTipoRecurso().getId().equals(recurso.getId()))
							.findFirst().orElse(null);
					if (tarifa == null) continue;
					
					Consumo c = new Consumo();
					c.setSede(lima);
					c.setTipoRecurso(recurso);
					c.setTarifa(tarifa);
					c.setUsuarioRegistro(admin);
					
					// Consumo aleatorio entre 500 y 1500
					double randomVal = 500 + Math.random() * 1000;
					c.setCantidadConsumida(new BigDecimal(randomVal).setScale(4, java.math.RoundingMode.HALF_UP));
					
					// Fecha aproximada del periodo
					int year = Integer.parseInt(periodo.split("-")[0]);
					int month = Integer.parseInt(periodo.split("-")[1]);
					c.setFechaConsumo(LocalDate.of(year, month, 15));
					c.setPeriodo(periodo);
					
					Consumo saved = consumoRepository.save(c);
					BigDecimal costoPen = saved.getCantidadConsumida().multiply(tarifa.getPrecioUnitarioPen());
					conversionFinancieraService.calcularYGuardarCostos(saved, costoPen);
				}
			}
		}
	}
